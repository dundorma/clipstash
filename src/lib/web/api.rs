...
impl<'r> FromRequest<'r> for ApiKey {
    ...
    async fn from_request(request: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        ...
        fn key_error(e: ApiKeyError) -> Outcome<ApiKey, ApiError> {
            Outcome::Failure((Status::BadRequest, ApiError::KeyErro(Json(e))))
        }

        match request.headers().get_one(API_KEY_HEADER) {
            None => key_error(ApiKeyError::NotFound("API key not found".to_owned())),
            Some(key) => {
                let db = match request.guard::<&State<AppDatabase>>().await {
...
pub async fn get_clip(
    ...
    let request = service::ask::GetClip {
    ...
    let clip = action::get_clip(request, database.get_pool()).await?;
